  ...
  try {
    ...
    if (result && result.Err === '') {
      ...
      try {
        ...
        // Kullanıcı adını HTML'den çek (farklı patternler dene)
        // Pattern 1: Hoşgeldiniz, AD SOYAD
        let nameMatch = homeHtml.match(/Hoşgeldin[^,]*,\s*([^<\n]+)/i);
        if (!nameMatch) {
          // Pattern 2: dropdown veya navbar'daki isim
          nameMatch = homeHtml.match(/class="[^"]*user[_-]?name[^"]*"[^>]*>([^<]+)/i);
        }
        if (!nameMatch) {
          // Pattern 3: navbar-text veya display-name
          nameMatch = homeHtml.match(/navbar-text[^>]*>([^<]+)/i);
        }
        if (!nameMatch) {
          // Pattern 4: Genel arama — "AD SOYAD" formatındaki büyük harfli metinler
          nameMatch = homeHtml.match(/<(?:span|div|a|strong|b)[^>]*>\s*([A-ZÇĞİÖŞÜ][a-zçğıöşü]+\s+[A-ZÇĞİÖŞÜ][a-zçğıöşü]+)\s*</);
        }
        if (!nameMatch) {
          // Pattern 5: Tüm büyük harfli isim
          nameMatch = homeHtml.match(/<(?:span|div|a|strong|b)[^>]*>\s*([A-ZÇĞİÖŞÜ]{2,}\s+[A-ZÇĞİÖŞÜ]{2,})\s*</);
        }

        if (nameMatch && nameMatch[1]) {
          const fullName = nameMatch[1].trim();
          nameHint = maskName(fullName);
          console.log('BTU kullanıcı adı bulundu:', fullName, '→', nameHint);
        } else {
